...
export function PendingPaymentWatcher() {
  const { activateAfterPayment } = useAuth()
  ...
  if (status === 'success' && successCode) {
    return <SuccessModal shouldRotate={shouldRotate} activationCode={successCode} onClose={() => setStatus(null)} />
  }
...
/** 支付成功弹窗 */
function SuccessModal({ activationCode, onClose, shouldRotate }) {
