...
export function Header({
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-50">
      <div className="relative flex items-center justify-between px-2 py-0.5 md:px-4 md:py-3 bg-gradient-to-b from-black/80 to-black/40">
        {/* 左侧：返回按钮 + 标题 */}
        <div className="flex items-center gap-1 md:gap-4">
          {/* 侧边栏切换按钮 */}
          <motion.button
            onClick={isModalOpen ? undefined : () => { playButtonClick(); onToggleSidebar(); }}
            className={`relative w-6 h-6 md:w-10 md:h-10 flex items-center justify-center transition-colors ${isModalOpen
          ...
          {activityId !== 'la101' && (
            ...
            >
              ...
              <div className="px-1 py-0.5 md:px-3 md:py-1.5 bg-black rounded-[6px] relative group transition duration-200 text-white hover:bg-transparent flex items-center gap-1 md:gap-2">
                <img
                ...
                />
                <span className="text-[10px] md:text-sm font-bold">股东炮抽奖！</span>
              </div>
            </motion.button>
          )}
          ...
          {/* 标题 */}
          <div>
            <h1 className="text-white text-[10px] md:text-lg font-bold">{activityName}</h1>
        ...
        <div className="flex items-center gap-1 md:gap-4">
          ...
          {isActivated && isMember && (
            ...
            >
              <div className="flex items-center bg-orange-600/20 border border-orange-500 text-orange-400 p-0.5 md:p-1 shadow-[0_0_8px_rgba(234,88,12,0.3)] backdrop-blur-sm">
                <Crown size={10} strokeWidth={2.5} className="md:w-3.5 md:h-3.5" />
              </div>
              {totalRemainingDays > 0 && (
                <span className="text-[8px] md:text-[10px] text-orange-300/80 font-mono">
          ...
          {isActivated ? (
            ...
            >
              ...
              <button
                ...
                className="text-slate-300 hover:text-white font-mono text-[10px] md:text-xs px-1 py-0.5 md:px-2 border border-slate-700 bg-black/40 hover:border-slate-500 transition-colors cursor-pointer max-w-[60px] md:max-w-[120px] truncate"
              >
            ...
            <motion.button
              ...
              className="bg-orange-600/80 hover:bg-orange-500 text-white px-1.5 py-0.5 md:px-3 md:py-1 text-[10px] md:text-xs font-bold hover:shadow-[0_0_10px_rgba(234,88,12,0.5)] transition-all flex items-center gap-1 border border-orange-500"
            >
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            {!isFlagshipGacha && !isCargoGacha && (
              <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-cyan-500/30">
                <img
                  src={currencyIconUrl}
                  alt="筹码"
                  className="w-4 h-4 md:w-6 md:h-6"
                />
                <span className="text-cyan-400 font-bold text-[10px] md:text-sm">{gameState.currency}</span>
                {/* 加号按钮 */}
                <button
                  onClick={() => { playButtonClick(); onOpenShop(); }}
                  className="ml-0.5 md:ml-1 w-3 h-3 md:w-5 md:h-5 flex items-center justify-center bg-cyan-500 hover:bg-cyan-400 rounded-full text-white text-sm md:text-lg font-bold transition-colors"
                >
                  +
                </button>
              </div>
            ...
            {isCargoGacha && (() => {
              ...
              return (
                <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-blue-500/30">
                  <img
                    src={buildCurrencyIconUrl(currencyId, activityConfig)}
                    alt={currencyName}
                    className="w-4 h-4 md:w-6 md:h-6"
                  />
                  <span className="text-blue-400 font-bold text-[10px] md:text-sm">{gameState.commonCurrency || 0}</span>
                  {/* 加号按钮 - 直接添加电池/遥控器 */}
                  {onAddBatteries && (
                    <button
                      onClick={onAddBatteries}
                      className="ml-0.5 md:ml-1 w-3 h-3 md:w-5 md:h-5 flex items-center justify-center bg-blue-500 hover:bg-blue-400 rounded-full text-white text-sm md:text-lg font-bold transition-colors"
                    >
                      +
                    </button>
                  )}
            ...
            {isCargoGacha && (() => {
              ...
              return (
                <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-emerald-500/30">
                  <img
                    src={buildCurrencyIconUrl(currencyId, activityConfig)}
                    alt={currencyName}
                    className="w-4 h-4 md:w-6 md:h-6"
                  />
                  <span className="text-emerald-400 font-bold text-[10px] md:text-sm">{gameState.currency || 0}</span>
                  {/* 加号按钮 */}
                  <button
                    onClick={() => { playButtonClick(); onOpenShop(); }}
                    className="ml-0.5 md:ml-1 w-3 h-3 md:w-5 md:h-5 flex items-center justify-center bg-emerald-500 hover:bg-emerald-400 rounded-full text-white text-sm md:text-lg font-bold transition-colors"
                  >
                    +
                  </button>
                </div>
            ...
            {/* 旗舰宝箱类：普通钥匙 */}
            {isFlagshipGacha && (
              <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-gray-500/30">
                <img
                  src={buildCurrencyIconUrl('currency_common_lootboxkey', activityConfig)}
                  alt="钥匙"
                  className="w-4 h-4 md:w-6 md:h-6"
                />
                <span className="text-gray-300 font-bold text-[10px] md:text-sm">{gameState.commonCurrency || 0}</span>
                {/* 加号按钮 */}
                {onAddCommonKeys && (
                  <button
                    onClick={() => { playButtonClick(); onAddCommonKeys(); }}
                    className="ml-0.5 md:ml-1 w-3 h-3 md:w-5 md:h-5 flex items-center justify-center bg-gray-500 hover:bg-gray-400 rounded-full text-white text-sm md:text-lg font-bold transition-colors"
                  >
                    +
                  </button>
                )}
            ...
            {/* 旗舰宝箱类：旗舰钥匙 */}
            {isFlagshipGacha && (
              <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-amber-500/30">
                <img
                  src={buildCurrencyIconUrl('currency_premium_lootboxkey', activityConfig)}
                  alt="旗舰钥匙"
                  className="w-4 h-4 md:w-6 md:h-6"
                />
                <span className="text-amber-400 font-bold text-[10px] md:text-sm">{gameState.currency}</span>
                {/* 加号按钮 */}
                <button
                  onClick={() => { playButtonClick(); onOpenShop(); }}
                  className="ml-0.5 md:ml-1 w-3 h-3 md:w-5 md:h-5 flex items-center justify-center bg-amber-500 hover:bg-amber-400 rounded-full text-white text-sm md:text-lg font-bold transition-colors"
                >
                  +
                </button>
              </div>
            ...
            {/* 人民币 */}
            <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-red-500/30">
              <svg className="w-4 h-4 md:w-6 md:h-6" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
              ...
              <span className="text-red-400 font-bold text-[10px] md:text-sm">{gameState.rmb}</span>
            </div>

            {/* 艺术硬币 */}
            <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-purple-500/30">
              <img
                src={`${CDN_BASE_URL}/assets/common-items/Artstorm.png${IMG_WEBP}`}
                alt="艺术硬币"
                className="w-4 h-4 md:w-6 md:h-6"
              />
              <span className="text-purple-400 font-bold text-[10px] md:text-sm">
                {getArtstormTotal()}
              </span>
            </div>
            ...
            {/* 旗舰宝箱类：美金 */}
            {isFlagshipGacha && (
              <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-green-500/30">
                <img
            ...
            {/* 黄金 */}
            <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-yellow-500/30">
              <img
                src={`${CDN_BASE_URL}/assets/common-items/Hard.png${IMG_WEBP}`}
                alt="黄金"
                className="w-4 h-4 md:w-6 md:h-6"
              />
              <span className="text-yellow-400 font-bold text-[10px] md:text-sm">
                {getGoldTotal()}
              </span>
            </div>
            ...
            {isFlagshipGacha && (
              <div className="flex items-center gap-1 md:gap-2 bg-black/60 rounded-full px-1.5 py-0.5 md:px-3 md:py-1.5 border border-blue-500/30">
                <img
                  ...
                  className="w-4 h-4 md:w-6 md:h-6"
                />
                <span className="text-blue-400 font-bold text-[10px] md:text-sm">
                  {getUpgradesTotal()}
                </span>
              </div>
            )}
