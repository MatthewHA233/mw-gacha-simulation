...
export function buildItemImageUrl(item, activityIdOrConfig) {
  ...
  if (item.rarity === 'epic' || item.rarity === 'legendary') {
    let folder
    if (item.type === '战舰' || item.type === '无人潜航器' || item.type === '无人舰艇') {
      // 战舰、无人潜航器、无人舰艇都使用 units_ships 路径
      folder = 'units_ships'
...
export function buildLootboxTicketUrl(type, activityId = null) {
  ...
  switch (type) {
    ...
    case 'clan':
      return `${basePath}/common_clan_lootbox_ticket.png${IMG_WEBP}`
    case 'clan_premium':
      return `${basePath}/premium_clan_lootbox_ticket.png${IMG_WEBP}`
    case 'clan_cosmetic':
      return `${basePath}/common_cosmetic_clan_lootbox_ticket.png${IMG_WEBP}`
    case 'event_common':
